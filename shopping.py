...
def load_data(filename):
    ...
    #DictReader本身會以第一列自動作為key，不須再告知誰是key
    with open(filename, mode='r') as file:
        ...
        for row in reader:
            evidence.append([
                ...
                int(row["Region"]),
                int(row["TrafficType"]),
                #將visitortype和weekend轉換成二元制，再存入檔案中
...
def evaluate(labels, predictions):
    ...
    #sensitivity，假設生病的話，所有陽性的病人被篩出的機率。也就是真的positive裡面篩出了多少
    #specificity，假設生病的話，所有陰性的人檢驗為陰性的機率，也就是真的negative裡面多少為陰性
