...
class NimAI():
    ...
    def choose_action(self, state, epsilon=True):
        ...
        # random.random()，生成一個0到1之間的浮點數
        # 雖然有點難，但其實拆解起來看是兩個條件
        # 如果要random choice，首先要epsilon是True，而且random的值要比self.epsilon小
        # 假設self.epsilon是0.1，隨機生成數字小於0.1的機率為10%，探索機率就是10%
        if epsilon and random.random() < self.epsilon:
            return random.choice(list(available_actions))

        # key = lambda action，也就是可以用dictionary裡面的action來進行排序
